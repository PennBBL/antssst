...
def brain_extraction(image,
	...
	# HARD CODED FILE PATH SO IT WORKS WITH PREDOWNLOADED DATA -- only works if modality == "t1"
        weights_file_name = os.path.join(antsxnet_cache_directory,'brainExtractionRobustT1.h5')

        if verbose == True:
            print("Brain extraction:  retrieving template.")

        # HARD CODED FILE PATH SO IT WORKS WITH PREDOWNLOADED DATA 
        reorient_template_file_name_path = os.path.join(antsxnet_cache_directory,'S_template3.nii.gz')
